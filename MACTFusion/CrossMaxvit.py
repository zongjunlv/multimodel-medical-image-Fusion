...
class RelativeSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        self.kv = nn.Linear(in_channels, in_channels * 2, bias=True)
        #
...
class MaxViTTransformerBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor, input_fu: torch.Tensor) -> torch.Tensor:
        ...
        input_partitioned_fu = self.partition_function(input_fu, self.grid_window_size)
        input_partitioned_fu = input_partitioned_fu.view(-1, self.grid_window_size[0] * self.grid_window_size[1], C)

        input_norm = self.norm_1(input_partitioned)
        input_fu_norm =  self.norm_1(input_partitioned_fu)

        attn_cross = self.attention(input_norm, input_fu_norm)
        # Perform normalization, attention, and dropout
        output = input_partitioned + self.drop_path(attn_cross)
