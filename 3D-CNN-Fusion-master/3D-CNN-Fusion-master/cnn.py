...
class attention_fusion(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        fea_c = fea_c.unsqueeze(dim=-1).unsqueeze(dim=-1).unsqueeze(dim=-1)
        fea_z = self.relu(self.conv1(fea_c))
        for i, conv in enumerate(self.conv2):
            vector = conv(fea_z).unsqueeze(dim=1)
            if i == 0:
        ...
        for i, conv in enumerate(self.conv4):
            feature = conv(fea_s).unsqueeze(dim=1)
            if i == 0:
...
class cnn(nn.Module):
    ...
    def __init__(self, argx):
        ...
        # self.res3D = Res3D(64, 64)

        self.fusion_block = attention_fusion(32, 4)
        self.relu = nn.ReLU(inplace=False)
...
class EnhancedCNN(nn.Module):
    def __init__(self, argx):
        ...
        # 参数配置
        kernel_size = 3
        base_channels = 16  # 增加基础通道数

        # 分支1处理流 (T1图像)
        self.branch1 = nn.Sequential(
            BasicConv(1, base_channels, kernel_size, stride=1, padding=(kernel_size - 1) // 2, relu=True),
            nn.BatchNorm3d(base_channels),
            DSC_3D(base_channels, base_channels * 2),
            nn.BatchNorm3d(base_channels * 2),
            Res3D(base_channels * 2, base_channels * 2)  # 添加残差连接
        )
        ...
        self.branch2 = nn.Sequential(
            ...
            DSC_3D(base_channels, base_channels * 2),
            nn.BatchNorm3d(base_channels * 2),
            Res3D(base_channels * 2, base_channels * 2)
        )

        # 特征融合模块
        self.fusion = attention_fusion(base_channels * 2, 4)  # 输入通道翻倍
        ...
        self.decoder = nn.Sequential(
            ...
            nn.BatchNorm3d(base_channels),
            nn.ReLU(),
            DSC_3D(base_channels, 1),
            nn.Sigmoid()  # 输出限制在[0,1]
        )

    def forward(self, x1, x2):
        # 分支处理
        b1 = self.branch1(x1)  # [B, 64, D, H, W]
        b2 = self.branch2(x2)  # [B, 64, D, H, W]

        # 特征拼接与融合
        fused = self.fusion(b1, b2)  # [B, 64, D, H, W]

        # 生成融合权重图
        weight_map = self.decoder(fused)  # [B, 1, D, H, W]

        # 加权融合输出
        outputs = weight_map * x1 + (1 - weight_map) * x2
        return outputs