...
class SS2D(nn.Module):
    ...
    def forward_corev0(self, x: torch.Tensor):
        ...
        # 把特征图拉成序列并拼出四个方向
        # 正向
        # x.view（）根据参数，将tensor做一个维度处理，x.view(B, -1, L)就是将X展平为3维（B，KC，L=H*W）
        # contiguous() 会返回一个在内存中连续存放数据的张量副本。
        # 如果原张量经过 permute、transpose 等操作变成了非连续布局，某些需要连续内存的函数（比如 .view()）就会报错，
        # 这时调用 .contiguous() 会重新拷贝数据，让它按当前形状在内存里顺序排列。
        # torch.stack([..., ...], dim=1) 把这两个序列沿新维拼成形状 (B, 2, C, L)
        x_hwwh = torch.stack([x.view(B, -1, L), torch.transpose(x, dim0=2, dim1=3).contiguous().view(B, -1, L)], dim=1).view(B, 2, -1, L)
        # 反向相加，一共四个方向
        xs = torch.cat([x_hwwh, torch.flip(x_hwwh, dims=[-1])], dim=1) # (b, k, d, l)

        # 在进入 selective scan 之前，为每个扫描方向生成状态空间模型所需的 Δ、B、C 参数
        # xs.view(B, K, -1, L) 把每个扫描方向的序列整理成 (batch, direction, channel, sequence_length)
        ...
        # 这些 dts/Bs/Cs reshape后 ，完成状态空间的选择性扫描，输出四个方向的响应
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.conv33conv33conv11 = nn.Sequential(
            # BN → 3×3 → BN → ReLU → 3×3 → BN → ReLU → 1×1 → ReLU
