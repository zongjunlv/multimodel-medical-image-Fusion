...
This codebase has been refactored with a clean and modular structure:
...
│   ├── medmamba.py           # Main MedMamba model
│   └── layers/               # Model components (SS2D, VSS layers, etc.)
...
```bash
# Basic training
python train.py --dataset custom --train_root /path/to/train --val_root /path/to/val

# Training with specific configuration
python train.py --model small --dataset kvasir --batch_size 64 --epochs 100
